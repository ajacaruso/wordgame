...
public class TileChooser extends JPanel implements ActionListener, GraphicsBankChangeListener
{
	...
	void createPropertiesDialog(JFrame dialogOwner) {
		...
		tileImg.setBorder(new TitledBorder("Image"));
                ...
                usable = new JCheckBox();
                special = new JComboBox<>();
                for(int i=0; i<WordGameProps.SpecialProps.size; i++)
                {
                    special.addItem(WordGameProps.SpecialProps.values()[i].toString());
                }
		...
		btns1.add(userText, c);
                ...
                c.gridx = 0;
                c.gridy = 4;
                btns1.add(new JLabel("Usable"), c);
                c.gridx = 1;
		btns1.add(usable, c);
                
                c.gridx = 0;
                c.gridy = 5;
                btns1.add(new JLabel("Special"), c);
                c.gridx = 1;
		btns1.add(special, c);
		...
		propertiesDialog.setSize(600, 500);
	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource() == applyBtn && propertyTile != null) {
			...
			propertyTile.info = userText.getText();
                        if(propertyTile.props == null)
                        {
                            propertyTile.props = new WordGameProps();
                        }
                        propertyTile.props.usable = usable.isSelected();
                        propertyTile.props.special = WordGameProps.SpecialProps.fromString(special.getSelectedItem().toString());
	...
	void showProperties(Tile t) {
		...
		if(t != null) {
			...
			tileType.setEditable(true);
                        ...
                        if(t.props != null)
                        {
                            usable.setSelected(t.props.usable);
                        }
                        usable.setEnabled(true);
                        if(t.props != null && t.props.special != null)
                        {
                            special.setSelectedItem(t.props.special.toString());
                        }
